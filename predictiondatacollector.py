...
def get_all_game_ids(year):
    schedule = mlb.get_schedule(start_date=f'{year}-04-01', end_date=f'{year}-08-19') 
    game_ids = []
...
def main():
    game_ids = get_all_game_ids(2024)
    all_game_data = []
    for count, game_id in enumerate(game_ids, start=1):
        game_data = get_game_data(game_id)
        all_game_data.extend(game_data)
        #log progress every 50 games for status
        if count % 50 == 0:
            print(f"Processed {count} games out of {len(game_ids)}")
            time.sleep(1)
    print("Data collection complete.")
    #save_to_csv(all_game_data, 'prediction_test_2024.csv')
    #going to check for timeout issues prior to creation of csv file
